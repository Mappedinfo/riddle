...
export default defineConfig(({ mode }) => {
  ...
  const isQwen = Boolean(env.QWEN_AI_KEY);
  const apiKey = env.QWEN_AI_KEY || env.DEEPSEEK_API_KEY || "";
  const apiBase = (env.QWEN_API_URL || env.DEEPSEEK_API_URL || "https://token-plan.cn-beijing.maas.aliyuncs.com/compatible-mode/v1").replace(/\/+$/, "");
